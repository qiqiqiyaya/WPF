    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ofd.Description = @"Browse to the directory to save the icons";
                    ofd.SelectedPath = saveDir;
                    if (ofd.ShowDialog() == DialogResult.OK)
                    {
                        saveDir = ofd.SelectedPath;
                    }
                    if (Directory.Exists(saveDir))
                    {
                        string exeFileFullName = iconPickerDialog.FileName;
                        string baseName = Path.GetFileNameWithoutExtension(exeFileFullName);
                        string ext = Path.GetExtension(exeFileFullName).ToLower();
                        if (ext == ".dll" || ext == ".exe")
                        {
                            IconExtractor extractor = new IconExtractor(exeFileFullName);
                            Icon[] allIcons = extractor.GetAllIcons();
                            for (int iconNr = 0; iconNr < allIcons.Length; iconNr++)
                            {
                                byte[] iconBytes = IconUtil.GetIconData(allIcons[iconNr]);
                                string iconFileName = Path.Combine(saveDir, $"{baseName}_{iconNr:D5}.ico");
                                File.WriteAllBytes(iconFileName, iconBytes);
                            }
                            string msg = $"Saved {allIcons.Length} icon files in directory {saveDir}";
